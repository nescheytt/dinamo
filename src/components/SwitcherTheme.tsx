...
export default function SwitcherTheme() {
  ...
  return (
    <div className="flex p-1 gap-1">
      <div className={`${!isDark && "bg-zinc-300"} rounded-full`}>
        <button
          type="button"
          className={`max-w-fit p-2 ${!isDark ? 'text-zinc-900' : 'text-zinc-600'} ${!isDark ? 'hover:text-zinc-200' : 'hover:text-zinc-200'}`}
      ...
      <div className={`${isDark && "bg-zinc-800"} rounded-full`}>
        <button
          type="button"
          className={`max-w-fit p-2 ${isDark ? 'text-zinc-200' : 'text-zinc-600'} hover:text-zinc-400`}
